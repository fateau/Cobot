...
struct RobotDeclareData {
	int		motorNum;
	int		gestureNum;
	eRobotType::e	robotType;
	double	refRoot[6];			//參考座標到基座的轉換矩陣
};
struct EcatMappingData {
	int	robotInd;
	int	axisInd;
};
struct DHtable
{
	double a			[MAX_MOTOR_PER_ROBOT];
	...
	eJointType::e jointType[MAX_MOTOR_PER_ROBOT];
	double deg2mmRatio;
...
struct MSPose	//Master-Slave 的一組Pose
{
	double M_pre_pose[6];
	double S_pre_pose[6];
	double M_target_pose[6];
	double S_target_pose[6];
};
struct URDFtable //PMC Modified 11412//1209
{
	// 機構參數 (Kinematics)
	double x[MAX_MOTOR_PER_ROBOT];    // x, y, z (mm) - 相對於上一軸的位移
	double y[MAX_MOTOR_PER_ROBOT];
	double z[MAX_MOTOR_PER_ROBOT];
	double Rx[MAX_MOTOR_PER_ROBOT];   // Rx, Ry, Rz (0 或 1) - 旋轉軸定義
	double Ry[MAX_MOTOR_PER_ROBOT];
	double Rz[MAX_MOTOR_PER_ROBOT];

	// 動力學參數 (Dynamics)
	double weight[MAX_MOTOR_PER_ROBOT];       // weight (kg) - 連桿質量
	double center_x[MAX_MOTOR_PER_ROBOT];
	double center_y[MAX_MOTOR_PER_ROBOT];
	double center_z[MAX_MOTOR_PER_ROBOT];
};
...
{
	double	maxV, maxA;
	double	contD;				  // 連續運動百分比，<=0 表示沒有連續
};
struct CmdMoveRaw
{
	eCmdFormat::e		format;		//pose, axis or point.
	eMovePathType::e	pathType;	//p2p, line or circle.
	eFrame::e			frame;		//Base or Tool
	BOOL				isAbs;		//abs or rel

	// --- 每次只用其中一部分 ---
	int scriptBaseIndex;
	int scriptToolIndex;
	int pointBaseIndex;						// for 記點
	int pointToolIndex;
...
struct CmdMove	//transferred from CmdMoveRaw
{
	eCmdFormat::e		format;		//pose, axis or point.
	eMovePathType::e	pathType;	//p2p, line or circle.
	...
	double    midPose[MAX_REDUNDANCY];
	double	  endPose[MAX_REDUNDANCY];

	double	startAxis[MAX_MOTOR_PER_ROBOT];
	double	  endAxis[MAX_MOTOR_PER_ROBOT];

	double	startVs[MAX_MOTOR_PER_ROBOT];
	double	  endVs[MAX_MOTOR_PER_ROBOT];
...
struct CmdBasicInfo
{
	int lineId;					//在劇本中的行數
	int funcId;					//副程式編號
	int myMasterId;				//正在跟隨的 Master編號。 -1 表示沒有跟隨任何Master。
	eScriptCmdType::e	type;	//路徑類型
};
struct ScriptRawCmd
{
	CmdBasicInfo info;

	CmdMoveRaw	cmdMoveRaw;
	CmdIo		cmdIo;
	CmdDelay	cmdDelay;
	CmdSync		cmdSync;
};
struct ScriptPathCmd //從 ScriptCmdRaw 計算而來
{
	CmdBasicInfo info;
...
struct IntpCmd //插補完的命令。可以在myCallBack直接喂給motor.
{
	CmdBasicInfo info;
	int toolIndex;

	eTargetFormat::e	format;
	double	axisDeg[MAX_MOTOR_PER_ROBOT];
	double	axisVel[MAX_MOTOR_PER_ROBOT];
	double	axisAcc[MAX_MOTOR_PER_ROBOT];

	double	Pose		 [MAX_REDUNDANCY];
	double	nowDs;			//距離起點已行走的距離
	double	nowVs;			//此瞬間的速度
...
// 手拉 //add by yunyu 20250423
struct HGParams
{
	double aidRatio[MAX_MOTOR_PER_ROBOT];
	double g_ratio[MAX_MOTOR_PER_ROBOT];
	double g_shift[MAX_MOTOR_PER_ROBOT];
	...
	int	   errorValue[MAX_MOTOR_PER_ROBOT];
	double ratedTorq[MAX_MOTOR_PER_ROBOT];
	int MaxTolerateTime; //add by yunyu 20250509
	double payload;
	//PMC Modified 11501 //0127
	double centerX;
	double centerY;
	double centerZ;
};